...
function buildSystemPrompt(summary: string, recommendations: CompactRecommendation[]): string {
...
## Recommendations
${lines || 'No recommendations identified yet.'}
...
export function createRecommendationNode(ollama: OllamaClient) {
  return async function recommendationNode(
    ...
    const system = buildSystemPrompt(ctx.summary, ctx.recommendations);
